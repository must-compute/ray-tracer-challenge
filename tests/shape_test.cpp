...
TEST(TestShape, IntersectingScaledShapeWithRay) {
  ...
  EXPECT_EQ(test_shape.local_ray()->origin(), Point(0.0, 0.0, -2.5));
  EXPECT_EQ(test_shape.local_ray()->direction(), Vector(0.0, 0.0, 0.5));
}
...
TEST(TestShape, IntersectingTranslatedShapeWithRay) {
  ...
  const auto xs = test_shape.intersect(ray);
  ASSERT_TRUE(test_shape.local_ray());
  EXPECT_EQ(test_shape.local_ray()->origin(), Point(-5.0, 0.0, -5.0));
  EXPECT_EQ(test_shape.local_ray()->direction(), Vector(0.0, 0.0, 1.0));
}

TEST(TestShape, ComputingNormalOnATranslatedShape) {
  auto test_shape = TestShape{};
  test_shape.set_transform(tf::translation(0.0, 1.0, 0.0));
  EXPECT_EQ(test_shape.normal_at(Point(0.0, 1.70711, -0.70711)), Vector(0.0, 0.70711, -0.70711));
}
...
TEST(TestShape, ComputingNormalOnTransformedShape) {
  ...
  EXPECT_EQ(test_shape.normal_at(Point(0.0, loc, -loc)), Vector(0.0, 0.97014, -0.24254));
}
